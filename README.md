...
You can also manage your own feeds from the dashboard — **Objects → Reference →
Custom abuse feeds → + New feed** — as named objects (a label plus one or more
URLs). They are appended to any `ABUSE_FEEDS` from `config` and deployed via
Commit. Each feed is capped at `ABUSE_FEEDS_MAX` entries (default 200000) so a
runaway list can't build a giant, slow set.
