...
## build: cross-compile the dashboard and QoS binaries for each arch into dist/
build:
	@mkdir -p $(DIST)
	@for a in $(ARCHES); do \
	  echo ">> nftgeo-ui linux/$$a"; \
	  GOOS=linux GOARCH=$$a $(GO) build -trimpath -ldflags "-s -w" -o $(DIST)/nftgeo-ui-linux-$$a ./ui; \
	  echo ">> nftgeo-qos linux/$$a"; \
	  GOOS=linux GOARCH=$$a $(GO) build -trimpath -ldflags "-s -w" -o $(DIST)/nftgeo-qos-linux-$$a ./qos; \
	done
...
lint:
	shellcheck -S warning --exclude=SC1090 bin/nftgeo-update bin/nftgeo tests/render/*.sh tests/migrate/*.sh tests/man/*.sh tests/examples/*.sh tests/integration/*.sh tests/package/*.sh
	@test -z "$$(gofmt -l ui/ qos/)" || { echo "gofmt needed:"; gofmt -l ui/ qos/; exit 1; }
...
units:
	@mkdir -p $(DIST)/units
	@for u in nftgeo.service nftgeo.timer nftgeo-ui.service nftgeo-qos.service; do \
